...
<html lang="en">
  <head>
    ...
    <title>Ajalatravel Route Table View</title>
    <!-- plugins:css -->
    ...
    <script>
      ...
      $(document).ready(function(){     
        $('.deleteRoute').on('click', function(){
         ...
         var delRoute = $(this).attr('id');
         delRoute = delRoute.split(' ');
         $('.del_route').attr('id', 'del_route'+delRoute[1])
         $('#del_route'+delRoute[1]).on('click', function(){
          value= {
           routeId: delRoute[1]
          }
          alert( $(this).attr('id'));
          $.ajax({
            ...
            if (result.success=='true'){
              ...
              $("#route-message").prepend("<div class='status alert alert-danger text-center col-sm-9 col-sm-offset-1' style='font-size:small;margin-top:10px'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times</a><strong >" +result.message+"</strong></div>"); 
             setTimeout(function(){
              location.reload();
             }, 6000);
            ...
            }else if(result.success=='noRoute'){
              $('#confirm-delete').modal('hide');
              $("#route-message").prepend("<div class='status alert alert-danger text-center col-sm-9 col-sm-offset-1' style='font-size:small;margin-top:10px'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times</a><strong >" +result.message+"</strong></div>"); 
             setTimeout(function(){
              location.reload();
    ...
    function Save(id) {
     ...
     var name1=document.getElementById(id_data1).value;
      if (name1==null || name1=='') {
        document.getElementById(messName).innerHTML="<span style='color:red;font-size:x-small'>Route name can't be blank</span>";
     }else if(!name1.match(/^[a-zA-Z]+$/)){
       document.getElementById(messName).innerHTML="<span style='color:red';margin-left:16px;font-size:x-small'> Route name is not letters</span>";
     }else{
      var parameter="name=" + name1 + "&addRouteA=" + 'save' + "&id=" + ids;
        ...
        var response=JSON.parse(xmlhttp.responseText);
         if(JSON.parse(response.success=="true")){
           ...
           document.getElementById(edit).style.display='block';
            ...
            if(JSON.parse(response.error.routeName !=undefined)){
              document.getElementById(messName).innerHTML='<span class="text-danger">'+response.error.routeName+'</span>';
            }
      ...
      xmlhttp.open("POST","ajax-add-route.php",true);
              ...
              <div class="col-lg-12 grid-margin stretch-card">
                <div class="card">
                  <div class="card-body" style="background-color:ghostwhite;height:560px;">
                    <h4 class="text-primary">Edit Table For All Routes</h4>
                    <p class="text-danger"> This this table is to make adjustment to any of<code class="text-primary">route</code> information</p>
                  ...
                  <div class="modal col-sm-8 col-sm-offset-4" id="confirm-delete" tabindex="-1" role="dialog" aria-labelledby="delete" aria-hidden="true">                  
                    <div class="modal-dialog">
                      <div class="modal-content">
                        ...
                        <h4 class="modal-title text-primary" id="Heading">Delete this Route</h4>
                        </div>
                        <div class="modal-body">
                          <div class="alert alert-danger  format"><span class="fa fa-warning text-danger"></span> Are you sure you want to delete this Route?</div>
                        </div>
                        <div class="modal-footer">
                          <button  class="btn btn-success del_route"><span class="fa fa-check-circle"></span> Yes</button>
